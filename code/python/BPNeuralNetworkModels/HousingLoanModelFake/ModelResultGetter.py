...
'''
risk item replacement for house loan and personal loan, should be deleted after real data is ready
'''
##################################################### begin #################################################################
...
for rowNumber in range(len(itemTypes)):
    if itemTypes[rowNumber] in ['1018']:
        riskItemList[rowNumber] = [item for item in riskItemList[rowNumber] if item.lower().find('car') < 0]
        if itemTypes[rowNumber] == '1018':
            if 'houseLoanBalance' not in riskItemList[rowNumber]:
                riskItemList[rowNumber].append('houseLoanBalance')
