...
def retuneScoreForCarLoanModel(header, data, rowNum):
    offset = 0
    checkList = [
        'guaranteeWay',
        'extraGuarantee'
    ...
    for key in checkMap.keys():
        if key == 'guaranteeWay' and checkMap[key] >= 0:
            offset += 100
...
# 1019: car loan
# 1018: house loan
mongoDataProcessor = MongoDataProcessor(['1019'], address=dbAddressIP)
...
for rowNumber in range(len(attributeSet)):
    ...
    score += retuneScoreForCarLoanModel(header, data, rowNumber)
    if score > 1000:
        score = 950
    if score < 50:
        score = 50
